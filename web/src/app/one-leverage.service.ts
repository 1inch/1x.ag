...
export class OneLeverageService {
    ...
    async openPosition(
        ...
        console.log(ethers.utils.bigNumberify(stopLoss).div(100).toNumber());
        console.log(ethers.utils.bigNumberify(1).sub(
        ...
        const callData = leverageContract.methods.openPosition(
            ...
            ethers.utils.bigNumberify(1e9).sub(
                ethers.utils.bigNumberify(stopLoss * 10000).mul(1e9).div(1e3).div(10000)
            ).mul(1e9),
            ethers.utils.bigNumberify(1e9).add(
                ethers.utils.bigNumberify(takeProfit * 10000).mul(1e9).div(1e3).div(10000)
            ).mul(1e9)
        )
            .encodeABI();
